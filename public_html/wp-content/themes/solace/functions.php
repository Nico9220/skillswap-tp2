...
// Crear un Custom Post Type para Cursos
function crear_posttype_cursos() {
    $labels = array(
        ...
        'not_found_in_trash' => __('No hay cursos en la papelera.')
    );
    ...
    $args = array(
        ...
        'show_in_rest'       => true, // para editor de bloques
        'menu_icon'          => 'dashicons-welcome-learn-more', // ícono de admin
    );

    register_post_type('curso', $args);
}
add_action('init', 'crear_posttype_cursos');


function reseñas_formulario() {
    if (!is_user_logged_in()) {
        return '<p>Debes estar logueado para dejar una reseña.</p>';
    }

    $mensaje = '';
    $post_id = get_the_ID();
    $user = wp_get_current_user(); // Usuario actual
    $user_id = $user->ID;

    if (isset($_POST['submit_reseña'])) {
        $texto = sanitize_textarea_field($_POST['reseña']);
        $puntaje = intval($_POST['puntaje']);

        // Insertamos el comentario con datos del usuario
        $commentdata = array(
            'comment_post_ID' => $post_id,
            'comment_author' => $user->display_name,  // nombre del usuario
            'comment_author_email' => $user->user_email, // email del usuario
        ...
        if ($comment_id) {
            add_comment_meta($comment_id, 'puntaje', $puntaje);
            $mensaje = '<p style="color:green;">¡Tu reseña fue enviada!</p>';
        }
    }

    $form = '<form method="post">';
    $form .= $mensaje;
    $form .= '<label for="reseña">Tu reseña</label><br>';
    $form .= '<textarea name="reseña" rows="4" cols="50" required></textarea><br>';
    $form .= '<label for="puntaje">Puntaje</label><br>';
    $form .= '<select name="puntaje" required>
                <option value="">--</option>
    ...
    $form .= '<input type="submit" name="submit_reseña" value="Enviar Reseña">';
    $form .= '</form>';

    return $form;
}
add_shortcode('reseñas_form', 'reseñas_formulario');

add_filter('comment_text', function($text, $comment) {
    $puntaje = get_comment_meta($comment->comment_ID, 'puntaje', true);
    if ($puntaje) {
        $text .= '<p>Puntaje: ' . str_repeat('⭐', $puntaje) . '</p>';
    }
    return $text;
}, 10, 2);


// Guardar y mostrar habilidades en un shortcode
function mis_habilidades_form() {
    if ( !is_user_logged_in() ) {
        return '<p>Debes estar logueado para cargar tus habilidades.</p>';
    }
    ...
    // Guardar habilidades
    if ( isset($_POST['submit_habilidades']) && isset($_POST['habilidades']) ) {
        $habilidades = sanitize_text_field($_POST['habilidades']);
        update_user_meta($user_id, 'habilidades', $habilidades);
        $mensaje = '<p style="color:green;">Tus habilidades se guardaron correctamente.</p>';
    }

    // Obtener habilidades actuales
    $valor_actual = get_user_meta($user_id, 'habilidades', true);

    // Mostrar habilidades guardadas
    $mostrar = '';
    if ( $valor_actual ) {
        $lista = explode(',', $valor_actual);
        $mostrar .= '<p><strong>Tus habilidades actuales:</strong></p><ul style="list-style:none;padding:0;">';
        foreach($lista as $h) {
            $mostrar .= '<li style="display:inline-block;background:#4CAF50;color:white;padding:5px 10px;margin:3px;border-radius:12px;">' . esc_html(trim($h)) . '</li>';
        }
        $mostrar .= '</ul>';
    }
    ...
    $form .= '<form method="post">';
    $form .= '<label for="habilidades">Tus habilidades (separadas por coma)</label><br>';
    $form .= '<textarea name="habilidades" id="habilidades" rows="4" cols="50">'. esc_textarea($valor_actual) .'</textarea><br><br>';
    $form .= '<input type="submit" name="submit_habilidades" value="Guardar Habilidades">';
    $form .= '</form>';

    return $form;
}
add_shortcode('mis_habilidades', 'mis_habilidades_form');
